  ...
  if (['discover', 'schema'].includes(request.action)) {
    ...
    if (request.action === 'discover') {
      ...
      const entries = new Map(hub.map(item => [item.machineName, {
        ...
        installed_versions: [], authoring_supported: TYPES.includes(item.machineName),
      }]));
      for (const [name, versions] of Object.entries(installed)) {
        ...
        if (!entries.has(name)) entries.set(name, { machine_name: name, title: runnable[0].title,
          hub_version: null, core_compatible: null, authoring_supported: TYPES.includes(name) });
      ...
      return { core: editor.config.h5pVersion, last_updated: await editor.contentTypeCache.getLastUpdate() || null,
        ...
        note: 'Hub compatibility is not Moodle verification. Only authoring_supported types can be generated by this MCP.' };
    }
    ...
    return { core: editor.config.h5pVersion, library: `${name} ${version.majorVersion}.${version.minorVersion}`,
      ...
      authoring_supported: TYPES.includes(name),
      note: 'Native H5P semantics, not JSON Schema. Nested library options refer to separate schemas; query those exact versions. Schema discovery does not enable generic export.' };
  }
